...
- [保守引き継ぎ会 スライド（PowerPoint）](./受付システム_保守引き継ぎ.pptx) — 引き継ぎ会用の説明資料
- **[AIエージェントによる保守ガイド](./ai-agent-maintenance.md)** — 各自の好みのAIエージェントでの保守（セットアップ〜指示〜確認）。共通ルールは直下の [`AGENTS.md`](../AGENTS.md)
- **[AIエージェント保守 ハンズオン＆実演ガイド](./ai-agent-hands-on.md)** — 引き継ぎ会でその場で実演できるシナリオ＋実際の変更例（過去日ガード・対応者表示・会議室同期トラブル）
