...
5. [カスタムドメインの設定（オプション）](#5-カスタムドメインの設定オプション)
6. [Google Apps Script の初期セットアップ](#6-google-apps-script-の初期セットアップ)
...
デフォルトの `https://xxxx.vercel.app` の代わりに独自ドメイン（例: `https://reception.your-company.com`）を使う場合の手順です。

### 5-1. Vercel にドメインを追加

1. Vercel ダッシュボード → **Settings → Domains** → **Add**
2. 取得済みのドメイン名を入力（例: `reception.your-company.com`）
3. Vercel が必要な DNS レコードを表示するのでメモしておく

### 5-2. DNS レコードを設定

ドメイン管理会社（Google Domains、Cloudflare、お名前.com など）の DNS 設定画面で、Vercel が指定するレコードを追加します。

| TYPE | ホスト名 | VALUE |
|------|---------|-------|
| A | `@`（ルートドメイン） | Vercel 指定の IP アドレス |
| CNAME | `www` | Vercel 指定の CNAME 値 |

> ⚠️ **具体的な値は Vercel の Domains 画面に表示されるものを使用してください。**  
> Vercel のインフラ変更に伴い値が更新されることがあります。画面上の値が正です。

### 5-3. ネームサーバーの確認

DNS レコードを追加しても Vercel で「Invalid Configuration」が続く場合は、**ネームサーバーが DNS レコード設定用のものになっていない**可能性があります。

多くのドメイン管理会社では、独自の「ホームページ用」ネームサーバーと「DNS レコード設定用」ネームサーバーが別になっています。DNS レコードを追加した画面のネームサーバーを使うように切り替えが必要です。

- 管理画面に「DNS レコード設定用ネームサーバーに変更する」などのオプションがあれば適用してください
- ネームサーバーを変更すると、同じドメインで使っていたメール設定などに影響する場合があります。ドメインでメールを使用していないか事前に確認してください

### 5-4. 反映の確認

DNS の浸透には **5 分〜最大 48 時間**かかります。  
Vercel → Settings → Domains で **Refresh** をクリックし、✅ **Valid Configuration** になれば完了です。

### 5-5. GAS の RECEPTION_URL を更新

カスタムドメインが有効になったら、GAS のコードも更新してコミット・再デプロイします：

```javascript
// gas/reception-calendar.gs
const RECEPTION_URL = "https://reception.your-company.com";  // カスタムドメインに変更
```

> `reception-system-five.vercel.app` のような旧 URL は引き続き有効なので、移行期間中は両方使えます。

---

## 6. Google Apps Script の初期セットアップ

Apps Script は **1 つのプロジェクト** に以下の機能をすべてまとめています。
